...
产品采用“逻辑多 Agent、工程模块化单体”的架构。增长主理人 Agent 负责拆目标和推进持久任务状态机，IP 建模师、趋势研究员、内容策略师、文案创作者、质检官和复盘师通过结构化契约协作。每个 Agent 有独立工具白名单、数据权限、成本预算、超时和输出 Schema，但首期不拆为独立微服务。

用户界面采用“对话 + 活文档”的 AI-native 形态。用户描述目标后，系统持续推进研究、策略、创作、质检和复盘；文案、证据、成片和复盘结果作为持续生长的任务产物嵌入任务流。人只在事实、策略、公开表达、成本和长期记忆等关键节点决策。

首期使用表格/链接导入完成平台数据回流，抖音、视频号 API 放到二期。首期以单台 4 核 8GB 服务器降低固定成本，通过异步队列、外部对象存储、异地备份和严格并发上限保证可恢复性。二期再引入负载均衡、多实例、托管主备数据库和平台数据连接器。
...
| 首期闭环 | 文案、真人/数字人生产、外部发布记录、数据导入、复盘与策略升级 |
...
| 角色 | 主要任务 | 核心权限 | 明确禁止 |
|---|---|---|---|
| 团长 | 提供事实、确认表达、选择真人/数字人、查看结果 | 查看和纠正自己的 IP；审批自己的内容；管理自己的照片、声音和授权 | 查看内部爆款原文、其他团长信息、内部策略评分 |
...
| 真人/数字人双路径 | Production：真人交接包与数字人 Adapter | 锁定稿可以选择任一路径；失败可切换 |
...
flowchart TB
    ...
    ORCH --> A5["质检官"]
    ORCH --> A6["复盘师"]
    A1 & A2 & A3 & A4 & A5 & A6 --> CTX["受控上下文装配器"]
    CTX --> IP["团长 IP 资产域"]
    CTX --> INTEL["内部内容智能域"]
    CTX --> RUN["任务 / 产物 / 实验域"]
    ORCH --> TOOLS["模型与工具 Gateway"]
    TOOLS --> LLM["云端大模型 / 搜索"]
    TOOLS --> AVATAR["数字人 / 语音 API"]
    TOOLS --> STORAGE["对象存储"]
...
6. **异步优先。** 模型、数字人、导入和复盘均为可暂停、可恢复的异步步骤。
...
| Production | 真人交接、数字人作业、供应商和成本 | production_jobs, provider_routes, media_assets | `submit_job()`, `production.job.completed` | 剪辑器和本地渲染 |
...
| 内容生产 | 生成真人交接包或调用数字人 | 提词、素材包、数字人作业和成片 | 选择路径；预算超限时批准 |
| 外部剪辑发布 | 在外部工具精剪和发布 | 成片、平台链接和发布记录 | 发布前最终确认 |
...
任一状态还可转入 `BLOCKED`、`CANCELLED` 或 `FAILED_RETRYABLE`。等待人工、供应商或导入时不占用 Worker。恢复时依据检查点继续，不重做已成功且不可重复的步骤。
...
ArtifactVersion ──> ProductionJob ──> MediaAsset
...
- BYOK 凭证以租户为边界加密存储；
- 平台密钥按租户套餐和预算分配额度；
- Provider Adapter 统一 `submit`、`get_status`、`cancel` 和 `fetch_result`；
...
| `POST /v1/production/jobs` | 创建数字人/交接任务 | 成本预检、授权检查、幂等 |
...
| 数字人失败 | 查询状态后重试；可切厂商或真人路径 | 重试、换方案、转真人 |
...
- 供应商密钥保存于外部 Secret Manager 或加密凭证表；
...
| 系统盘/数据盘 | 推荐 200GB SSD；应用和数据库不存原始视频 |
| 网络 | 公网 HTTPS；安全组仅开放 80/443，SSH 限制管理来源 |
| 操作系统 | 受支持的 Linux LTS |
| 外部服务 | S3 兼容对象存储、邮件/短信/企业通知、云端模型、数字人 API、外部监控 |
...
├── Object Storage
├── Secret Manager
├── Model / Search / Digital Human APIs
...
- Worker 杀进程、模型限流、数字人超时和导入重复具备故障注入测试。
...
| P3 生产与复盘 | 一个数字人 Adapter、成片归档、数据导入、统一指标、复盘和策略升级 | 两个真实招商周期跑通；失败降级和导入回滚通过 |
...
| 数字人供应商失败/涨价 | 生产中断和成本增加 | Adapter、预算、熔断、备用供应商和真人降级 |
