...
3. 面向招商转化的选题方向、结构模板和质量标准；
4. 从选题、文案、发布数据到模板效果优化的实验闭环。
...
第一版只交付内容闭环：团长 IP 建档、内部内容情报、选题方向匹配、同方向候选文案、事实/质量检查、确认与锁稿、人工发布记录、平台数据导入、复盘和记忆升级。数字人口播短视频、MiniMax/HeyGen 编排、自动发布和平台数据 API 均不进入第一版，后续是否实现由业务验证后单独决策。首期以单台 4 核 8GB 服务器降低固定成本，通过异步队列、外部对象存储、异地备份和严格并发上限保证可恢复性。二期再考虑负载均衡、多实例和托管主备数据库。
...
| 首期业务 | 招商内容闭环；带货作为后续内容分类和模板包复用底层能力 |
| 首期闭环 | 团长 IP、内部内容情报、选题方向选择、同方向候选文案选择、质检/确认、人工发布记录、数据导入、复盘与模板效果优化 |
...
- 让每条文案都能追溯到团长 IP 事实、所选方向、内部结构模板版本和确认记录；
...
- **结构模板：** 由内部爆款内容人工拆解形成的抽象文案结构，包含适用 IP、方向、钩子、正文步骤、证据要求、CTA、平台规则和禁用条件，不包含供生成模型直接复用的爆款原文。
...
| 第一版实现 | 后续待定，不实现 |
|---|---|
| 租户与角色、团长 IP、内部内容情报、结构模板库、选题方向选择、同方向候选文案选择、事实/质量检查、运营/团长确认、人工发布记录、CSV/XLSX 指标导入、统一指标、复盘、记忆提案、审计、成本与单机备份 | MiniMax/HeyGen、数字人档案、音频/视频资产、成片页面、自动发布、抖音/视频号 API、带货模板包、微信小程序、任务星图、多实例高可用 |
...
| 内容情报员 | 采集并拆解近 7 日爆款 | 访问内部来源、拆解和结构模板发布流程 | 访问团长敏感资料或外部开放内部原文 |
...
| 内部 7 日爆款内容工厂 | Content Intelligence：采集队列、拆解、复核、结构模板、过期淘汰 | 团长接口无法读取原文；运营可以每日完成采集和模板发布 |
| 招商文案生产 | Topic + Template + Artifact：方向候选、所选方向、同方向候选文案和平台变体 | 只能在用户选择一个方向后生成多篇候选稿；文案绑定 IP、方向、模板和事实证据版本 |
...
1. **核心自有，非核心适配。** IP、内容情报、选题匹配、结构模板、实验和审计属于我方；模型、数字人、剪辑和平台能力通过可替换 Adapter 使用。
...
| Content Intelligence | 受保护来源、拆解、模式和人工发布的结构模板 | source_items, decompositions, patterns, writing_template_versions | `match_templates()`, `writing_template.published` | 团长事实、方向选择和文案确认 |
...
3. **效果校准：** 用平台、受众和招商结果调整“IP 属性 × 选题方向 × 结构模板”的匹配权重，不得根据播放量改写身份、经历或价值观。
...
8. 来源被删除、权利状态变化或事实失效时，触发关联结构模板复查。
...
第一版由内容团队人工维护 20～30 个高质量结构模板，不让模型直接从爆款原文自动发布模板。原文库与结构模板库分离：原文只用于内部拆解和复核，生成侧只能读取已发布模板的抽象字段。
...
用户必须明确选择一个 `TopicDirectionCandidate`。选择后，系统只在该方向下匹配结构模板并生成默认 3 篇完整候选文案；禁止在一次候选集中混入多个方向。三篇稿件可以使用不同钩子或结构，但必须共享同一个 `topic_selection_id`，让用户解决“今天拍哪一篇”，而不是在多个方向和多个稿件组成的矩阵中盲选。
...
| 复盘升级 | 对比 IP 属性、方向、模板、平台、钩子和 CTA | 复盘卡、模板效果和记忆提案 | 运营发布模板效果结论；团长发布长期表达偏好 |
...
| 内容经验记忆 | 选题方向、结构模板、适用范围和验证结论 | 运营审批 | 版本化，带证据和有效期 | 按 IP 属性、平台、人群和业务过滤 |
...
1. 租户、角色和数据域权限过滤；
2. 锁定任务使用的 IP 快照、用户所选方向、结构模板和平台规则版本；
3. 结构化条件检索；
4. 关键词与向量混合召回；
5. 适用范围和时效重排；
6. 引用去重和证据状态检查；
...
- 任何模型、提示词、选题匹配规则或结构模板升级先在离线黄金集评估，再按金丝雀范围进入线上。
...
| P2 内容闭环 | 受保护原文库、20～30 个人工结构模板、IP 匹配方向、单方向候选文案、质检、确认和真人交接 | 运营独立完成 30 条“选方向 → 选文案”任务；候选批次跨方向混稿为 0；内部原文泄露为 0 |
| P3 发布数据与复盘 | 人工发布记录、数据导入、统一指标、复盘和模板效果/记忆升级 | 两个真实招商周期跑通；导入校验、去重和回滚通过 |
...
| Agent 自我强化错误 | 错误内容经验进入长期记忆 | 记忆提案必须审批；带证据、范围、版本和撤销 |
