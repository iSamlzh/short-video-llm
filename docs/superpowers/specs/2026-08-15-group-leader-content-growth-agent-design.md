...
- 行业由用户主动选择，Agent 不推荐、不猜测。
- 首版包含健康养生、美容护肤、母婴育儿、食品生鲜、家居日用、服饰穿搭、本地生活与实体门店、教育培训与知识服务、创业经营与商业服务、其他十个行业。
- 每个行业至少维护 30 道已启用问题；每次建档按画像覆盖规则选择 7 至 10 道，不随机抽取，不在问答阶段调用模型。
- 问题必须映射到画像字段和选题输入，重点获得目标受众、用户问题、核心观点、真实素材、表达方式、产品连接、行动引导、表达边界和长期选题方向。
- 画像中的事实和选题方向必须能追溯到原始问题及回答；修改答案后创建新的待确认画像版本。
- 完整数据结构、状态机、接口、异常处理与验收规则见 [《IP 建档问题库与内容画像详细设计》](2026-08-18-IP建档问题库与内容画像详细设计.md)。
