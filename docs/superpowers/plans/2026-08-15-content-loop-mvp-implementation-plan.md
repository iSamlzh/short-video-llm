...
> **For Codex:** REQUIRED SKILL: Use superpowers:executing-plans or superpowers:subagent-driven-development to implement this plan task by task.
...
**Architecture:** 单台 4C8G 服务器部署 Next.js、FastAPI、Celery、PostgreSQL/pgvector 与 Redis。确定性的 `Workflow Runtime` 负责编排状态、等待人工选择、幂等、重试和成本控制；首版只注册 `IP Agent`、`Content Agent`、`Quality & Learning Agent` 三个 Agent。内容生成与质量判断隔离，爆款原文不进入生成上下文；发布数据先以人工记录和 CSV/XLSX 导入回流。
...
- 角色：`operator`、`content_owner`、`group_leader`、`admin`。
...
短期访问令牌携带 `tenant_id`、`sub`、`role`；API 依赖将其解析为不可变 `TenantContext`。内容负责人可维护模板、质量标准和记忆；团长可维护自己的 IP、选择方向/文案、确认锁稿和查看结果。
...
def test_profile_version_requires_human_confirmation(service, operator):
    with pytest.raises(ApprovalRequired):
        service.publish(operator, draft_id="draft-1")
```
...
- `POST /api/ip-profiles/{id}/evidence-upload-url`
- `POST /api/ip-profiles/{id}/answers`
- `POST /api/ip-profiles/{id}/calibrations/{stage}/confirm`
- `POST /api/ip-profiles/{id}/versions/{version_id}/confirm`
- `POST /api/ip-profiles/{id}/versions/{version_id}/activate`
- `GET /api/ip-profiles/{id}/versions`
...
def test_only_content_owner_can_publish_structure(operator, service):
    with pytest.raises(Forbidden):
        service.publish(operator, draft_id="structure-draft-1")
```
...
- `POST /api/content-intelligence/sources`
- `POST /api/content-intelligence/structures/drafts`
- `POST /api/content-intelligence/structures/{id}/publish`
- `GET /api/content-intelligence/structures?status=published`
...
注册表对 Agent 与 mode 做静态映射，未知 Agent/mode 启动即失败。上下文构建器按 mode 读取最小字段并记录版本 ID；任何自由文本注入必须带来源类型和允许策略。
...
- `GET /api/runs/{id}/topic-directions`
- `POST /api/runs/{id}/topic-selection`
...
- `GET /api/runs/{id}/script-candidates`
- `POST /api/runs/{id}/script-selection`
- `POST /api/artifacts/{id}/confirm`
- `GET /api/artifacts/{id}/quality-report`
...
默认界面突出“下一步该做什么”，先用卡片比较 3–5 个方向，再展示选中方向下的 3 篇完整文案。状态、成本、质量标准和内部 Agent 来源默认折叠；不使用传统 SaaS 的多级侧栏、数据大盘和模块宫格作为主体验。
...
**Contracts:**
- 两阶段 API：`POST /api/metric-imports/preview` 和 `POST /api/metric-imports/{batch_id}/commit`。
...
- `GET /api/runs/{id}/review-and-learning`
- `POST /api/memory-proposals/{id}/decisions`
- `GET /api/quality-standard-proposals`
- `POST /api/quality-standard-proposals/{id}/activate`
- `POST /api/quality-standards/{version_id}/rollback`
...
def test_cross_tenant_run_is_not_found(client_t1, tenant_t2_run):
    response = client_t1.get(f"/api/runs/{tenant_t2_run.id}")
...
CI 依次运行 Python 格式/静态检查、TypeScript lint/typecheck、单元测试、PostgreSQL 集成测试、Alembic 正向升级、Playwright 核心闭环和 `docker compose config`。失败即阻止合并；不包含压测、故障注入、灾备演练或发布编排。

- [ ] **Step 5: 执行 4C8G 单机完整闭环验收**

Run: `docker compose -f ops/compose.yaml up -d --build`
Expected: PASS，全部容器健康，配置资源上限总和不超过 8GB。
