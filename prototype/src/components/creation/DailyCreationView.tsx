  ...
  return <div className="document-page daily-creation-view">
    ...
    <div className="document-grid">
      <article className="primary-document script-document">
        ...
        {paragraphs.map((paragraph, index) => <section className="script-paragraph" key={index}>{editing ? <textarea aria-label={`第 ${index + 1} 段`} value={paragraph} onChange={(event) => setParagraphs((current) => current.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} /> : <p>{paragraph}</p>}<PencilSimple size={21} weight="light" /></section>)}
