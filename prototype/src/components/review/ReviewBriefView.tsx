...
export function ReviewBriefView({ brief, onImport, onConfirm }: {
  brief: any
  onImport?: () => void
  onConfirm?: (input: { keep: string[]; avoid: string[]; nextContentSignals: string[] }) => Promise<void> | void
}) {
  ...
  return <div className="document-page review-view">
    ...
    <div className="document-grid"><article className="primary-document review-document real-review-document">
      <section><h2>能确定什么</h2>{payload.observations.length ? <ol className="review-observations">{payload.observations.map((item: any, index: number) => <li key={`${index}-${item.text}`}><p>{item.text}</p><span>证据 {item.evidenceSnapshotIds.map((id: string) => <button type="button" key={id}>{id}<ArrowSquareOut size={13} /></button>)}</span></li>)}</ol> : <p>当前还没有足够的已关联真实数据。</p>}</section>
    ...
    </article><MemoryPreview brief={brief} onConfirm={onConfirm} /></div>
  </div>
}
