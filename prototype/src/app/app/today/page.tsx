...
export default async function TodayPage() {
  ...
  const current = getAppDatabase().prepare(`SELECT ip_profile_id FROM user_current_context
    WHERE user_id=? AND tenant_id=?`).get(access.userId, access.tenantId) as { ip_profile_id: string } | undefined
  ...
  const accounts = current ? (getAppDatabase().prepare(`SELECT id,platform,account_name FROM content_accounts
    WHERE tenant_id=? AND ip_profile_id=? AND status='active' ORDER BY created_at,id`).all(
    access.tenantId, current.ip_profile_id,
