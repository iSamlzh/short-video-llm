...
这是一个本地运行的纵向原型，用来验证“最小 IP 信息 → 选题方向 → 同方向 3 篇文案 → 独立 QA → 锁稿 → 模拟表现 → 模型复盘”的内容闭环。它不是正式首版系统，也不包含登录、租户、角色权限、平台 API、数字人视频或生产级队列。
...
默认数据库为 `.data/prototype.sqlite`，刷新页面会从 SQLite 恢复当前 Run。需要重置时，停止开发服务器后删除这个单一原型数据库文件即可；其中只存用户输入、版本化产物、模拟指标和复盘，不存 API Key。

所有发布指标均由确定性模拟器生成，页面固定显示：**模拟数据，不代表真实平台表现**。复盘 Prompt 和服务端校验都禁止把这些指标表述为真实平台因果。

## 演示顺序

1. 填写六项最小 IP 信息，Agent 自动生成 3–5 个符合自身属性的方向。
2. 只选择一个“今天拍什么”的方向。
3. 围绕该方向生成恰好 3 篇完整口播稿，并选择一篇。
4. 运行独立 QA；硬门禁通过后锁稿。
