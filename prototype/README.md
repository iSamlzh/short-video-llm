...
首版采用一个 Next.js Node.js 进程、systemd、Nginx 和 SQLite Online Backup。代码库提供存活/就绪接口、全 API 结构化请求日志、跨 Nginx/API/模型任务的 `requestId`、正常日志 14 天与异常日志 90 天轮转策略、单实例服务文件、HTTPS 反向代理、部署脚本、每日/每周备份保留策略及禁止覆盖生产库的隔离恢复工具。
