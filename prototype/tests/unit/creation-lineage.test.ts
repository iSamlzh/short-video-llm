...
describe("CreationLineageRepository", () => {
  it("keeps one current daily run inside the exact tenant/IP/account scope", () => {
    ...
    database.exec(`CREATE TABLE creation_run_context (
      ...
      ip_profile_id TEXT NOT NULL, ip_profile_version INTEGER, content_account_id TEXT, business_date TEXT NOT NULL,
      tenant_memory_version INTEGER, structure_version_ids_json TEXT NOT NULL DEFAULT '[]',
      primary_structure_version_id TEXT, supporting_structure_version_ids_json TEXT NOT NULL DEFAULT '[]',
    ...
    repository.attach({ runId: "run-1", tenantId: "tenant-1", actorUserId: "owner", ipId: "ip-1", ipProfileVersion: 1, accountId: "account-1", businessDate: "2026-08-17", tenantMemoryVersion: 2, structureVersionIds: ["template-v1"] })
    repository.attach({ runId: "run-2", tenantId: "tenant-1", actorUserId: "owner", ipId: "ip-1", ipProfileVersion: 1, accountId: "account-1", businessDate: "2026-08-17", tenantMemoryVersion: 3, structureVersionIds: ["template-v2"] })
    ...
    repository.assignStructures("run-1", {
      primaryStructureVersionId: "template-v1",
      supportingStructureVersionIds: ["template-v2"],
    })
    expect(repository.get("run-1")?.primaryStructureVersionId).toBe("template-v1")
    expect(repository.get("run-1")?.supportingStructureVersionIds).toEqual(["template-v2"])
