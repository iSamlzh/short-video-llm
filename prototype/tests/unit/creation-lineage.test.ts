...
describe("CreationLineageRepository", () => {
  it("keeps one current daily run inside the exact tenant/IP/account scope", () => {
    ...
    database.exec(`CREATE TABLE creation_run_context (
      ...
      ip_profile_id TEXT NOT NULL, content_account_id TEXT, business_date TEXT NOT NULL, created_at TEXT NOT NULL
    )`)
    const repository = new CreationLineageRepository(database)
    repository.attach({ runId: "run-1", tenantId: "tenant-1", actorUserId: "owner", ipId: "ip-1", accountId: "account-1", businessDate: "2026-08-17" })
    repository.attach({ runId: "run-2", tenantId: "tenant-1", actorUserId: "owner", ipId: "ip-1", accountId: "account-1", businessDate: "2026-08-17" })
